...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (!extensions || !includeFolders) return;
...
async function loadConfig(rootPath: string): Promise<Config> {
  ...
  const globalConfigPath = path.join(
    os.homedir(),
    ".file-content-combiner.json"
  );
  const projectConfigPath = path.join(rootPath, "file-content-combiner.json");
...
async function processFiles(
  ...
  // Process each include folder recursively
